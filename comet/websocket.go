...
func ServeWebSocket(w http.ResponseWriter, req *http.Request) {
	...
	//通过 Upgrader.Upgrade method 方法获得一个链接
	//然后可以调用connection's WriteMessage and ReadMessage
...
func (server *Server) serveWebsocket(conn *websocket.Conn, tr *itime.Timer) {
	...
	for {
		...
		} else if p.Operation == define.OP_OFFLINE_SMS {
			if err = server.operator.PushOffLineMessage(p); err != nil {
				break
			}
		} else if p.Operation == define.OP_CLIENT_SMS_GET_SUCCESS {
			if err = server.operator.ClientGetMessageSuucss(p); err != nil {
				break
			}
		} else {
			// fmt.Println("其他")
...
func (server *Server) authWebsocket(conn *websocket.Conn, p *proto.Proto) (key string, rid int32, heartbeat time.Duration, err error) {
	...
	if key, rid, heartbeat, err = server.operator.Connect(p); err != nil {
		// fmt.Println("authWebsocket 鉴权失败：", err)
		return
	}

	p.Body = []byte("{\"Code\":1, \"msg\":\"auth success\"}")
